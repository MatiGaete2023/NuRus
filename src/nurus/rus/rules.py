...
def as_date(value: object) -> date | None:
    ...
    if isinstance(value, date):
        return value
    if isinstance(value, (int, float)) and not isinstance(value, bool) and 30000 < value < 60000:
...
def no_follow_up(value: object) -> bool:
    ...
    return any(text.startswith(prefix) for prefix in ("opd", "dam", "salud privada", "hospital", "unidad de salud mental", "cesfam", "red salud", "consulta externa", "colegio", "chile crece contigo"))
...
def _curator_and_heard(record: SourceRecord, columns: Mapping[str, str], catalog: dict, today: date) -> tuple[list[str], list[str]]:
    row, fragments, ids = record.values, [], []
    curator_column = columns.get("curador")
    if curator_column and not (_RUT.search(str(row.get(curator_column, "") or "")) or "Institución:" in str(row.get(curator_column, "") or "")):
        fragments.append(render(catalog, "COMUN", "CURADOR")); ids.append("T-01")
    heard_column = columns.get("oido")
    heard = as_date(row.get(heard_column)) if heard_column else None
    if heard and 0 <= (today - heard).days <= 45:
        fragments.append(render(catalog, "COMUN", "OIDO", FECHA_OIDO=format_date(heard))); ids.append("T-02")
    return fragments, ids


def _audience(record: SourceRecord, columns: Mapping[str, str], catalog: dict, today: date) -> tuple[list[str], list[str]]:
    column = columns.get("prox_aud")
    hearing = as_date(record.values.get(column)) if column else None
    if hearing and hearing >= today:
        return [render(catalog, "COMUN", "PROX_AUDIENCIA", FECHA_AUDIENCIA=format_date(hearing))], ["T-03"]
    return [], []


def evaluate_waiting(record: SourceRecord, columns: Mapping[str, str], catalog: dict, today: date) -> tuple[str, tuple[str, ...], tuple[Issue, ...]]:
    ...
    birth = as_date(row.get(columns.get("nacimiento", ""))) if columns.get("nacimiento") else None
    first = _first_name(row, columns)
    if birth and _age(birth, today) >= 18:
        fragments.append(render(catalog, "COMUN", "MAYORIA_EDAD", PNOMBRE=first, FECHA_MAYORIA=format_date(_adult_date(birth)))); ids.append("E-02")
        extra, extra_ids = _curator_and_heard(record, columns, catalog, today); fragments += extra; ids += extra_ids
        audience, audience_ids = _audience(record, columns, catalog, today); fragments += audience; ids += audience_ids
        return _compose(prefix(row, columns), fragments), tuple(ids), tuple(issues)
    if birth and 1 <= (_adult_date(birth) - today).days <= 60:
        fragments.append(render(catalog, "COMUN", "PROXIMA_MAYORIA", PNOMBRE=first, FECHA_MAYORIA=format_date(_adult_date(birth)))); ids.append("E-03")
    principal = False
    resolution = as_date(row.get(columns.get("resolucion", ""))) if columns.get("resolucion") else None
    if resolution and 0 <= (today - resolution).days <= 29:
        fragments.append(render(catalog, "ESPERA", "E04_RESOLUCION_RECIENTE", PROGRAMA=program_title(program), FECHA_RESOLUCION=format_date(resolution))); ids.append("E-04"); principal = True
    ...
    if wait is not None and wait >= 30:
        if is_dce(program):
            fragments.append(render(catalog, "ESPERA", "E05_SOLO_CORREO")); ids.append("E-05"); principal = True
        elif court in {"LAJA", "MULCHEN"}:
            fragments.append(render(catalog, "ESPERA", "E05_PROYECTO_Y_CORREO")); ids.append("E-05"); principal = True
        elif court == "TOME":
            fragments.append(render(catalog, "ESPERA", "E05_PROYECTO_Y_CORREO" if wait >= 60 else "E05_SOLO_CORREO")); ids.append("E-05"); principal = True
        else:
            issues.append(_issue(record, "E-05", "Tribunal no reconocido; regla omitida (G-06)."))
    if not principal:
        fragments.append(render(catalog, "ESPERA", "E06_SIN_RESOLUCION", PROGRAMA=program_title(program))); ids.append("E-06")
    extra, extra_ids = _curator_and_heard(record, columns, catalog, today); fragments += extra; ids += extra_ids
    audience, audience_ids = _audience(record, columns, catalog, today); fragments += audience; ids += audience_ids
    return _compose(prefix(row, columns), fragments), tuple(ids), tuple(issues)


def evaluate_reports(record: SourceRecord, columns: Mapping[str, str], catalog: dict, today: date) -> tuple[str, tuple[str, ...], tuple[Issue, ...]]:
    row = record.values
    program = row.get(columns.get("programa", ""), "")
    if no_follow_up(program):
        return _compose(prefix(row, columns), [render(catalog, "COMUN", "NO_SEGUIMIENTO", PROGRAMA=program_title(program))]), ("E-01",), ()
    due_column = columns.get("vencimiento")
    due = as_date(row.get(due_column)) if due_column else None
    if not due:
        return "", (), (_issue(record, "I-01/I-02", "Sin fecha de vencimiento (G-05)."),)
    days = (due - today).days
    if days < 0:
    ...
    audience, audience_ids = _audience(record, columns, catalog, today)
    return _compose(prefix(row, columns), fragments + audience), tuple([rule_id] + audience_ids), ()


def evaluate_compliance(record: SourceRecord, columns: Mapping[str, str], catalog: dict, today: date, cross_due: date | None = None) -> tuple[str, tuple[str, ...], tuple[Issue, ...]]:
    row, fragments, ids, issues = record.values, [], [], []
    program = row.get(columns.get("programa", ""), "")
    if no_follow_up(program):
        return _compose(prefix(row, columns), [render(catalog, "COMUN", "NO_SEGUIMIENTO", PROGRAMA=program_title(program))]), ("E-01",), ()
    ...
    first = _first_name(row, columns)
    if birth and _age(birth, today) >= 18:
        fragments.append(render(catalog, "COMUN", "MAYORIA_EDAD", PNOMBRE=first, FECHA_MAYORIA=format_date(_adult_date(birth)))); ids.append("C-01")
        extra, extra_ids = _curator_and_heard(record, columns, catalog, today); fragments += extra; ids += extra_ids
        audience, audience_ids = _audience(record, columns, catalog, today); fragments += audience; ids += audience_ids
        return _compose(prefix(row, columns), fragments), tuple(ids), tuple(issues)
    if birth and 1 <= (_adult_date(birth) - today).days <= 60:
        fragments.append(render(catalog, "COMUN", "PROXIMA_MAYORIA", PNOMBRE=first, FECHA_MAYORIA=format_date(_adult_date(birth)))); ids.append("C-02")
    ...
    entry = as_date(row.get(columns.get("ingreso", ""))) if columns.get("ingreso") else None
    principal = False
    if days_compliance is not None and 0 <= days_compliance <= 30:
        if entry:
            fragments.append(render(catalog, "CUMPLIMIENTO", "C03_INGRESO_RECIENTE", PROGRAMA=program_title(program), FECHA_INGRESO=format_date(entry))); ids.append("C-03"); principal = True
        else:
    ...
    projected_exit = as_date(row.get(columns.get("egreso_proy", ""))) if columns.get("egreso_proy") else None
    ...
    if expired:
        fragments.append(render(catalog, "CUMPLIMIENTO", "C04_VENCIDA", FECHA_EGRESO_PROYECTADO=format_date(projected_exit))); ids.append("C-04"); principal = True
    ...
    if ending:
        ...
        fragments.append(render(catalog, "CUMPLIMIENTO", key, FECHA_EGRESO_PROYECTADO=format_date(projected_exit))); ids.append("C-05"); principal = True
    if cross_due and not expired and not ending:
        fragments.append(render(catalog, "CUMPLIMIENTO", "C10_HOJA2", PROGRAMA=program_title(program), FECHA_VENCIMIENTO=format_date(cross_due))); ids.append("C-10"); principal = True
    extra, extra_ids = _curator_and_heard(record, columns, catalog, today); fragments += extra; ids += extra_ids
    program_normalized = normalize(program)
    individual_column = columns.get("ficha_ind")
    individual = as_date(row.get(individual_column)) if individual_column else None
    if program_normalized.startswith(("rta", "rtt", "res", "rfa", "rva")) and individual_column:
        if not individual:
            fragments.append(render(catalog, "CUMPLIMIENTO", "C07_SIN_FICHA")); ids.append("C-07")
        else:
            age = (today - individual).days
            if age > 180:
                fragments.append(render(catalog, "CUMPLIMIENTO", "C07_FICHA_ANTIGUA", FECHA_FICHA_INDIVIDUAL=format_date(individual))); ids.append("C-07")
            elif 0 <= age <= 30:
                fragments.append(render(catalog, "CUMPLIMIENTO", "C07_FICHA_RECIENTE", FECHA_FICHA_INDIVIDUAL=format_date(individual))); ids.append("C-07")
    fae_column = columns.get("ficha_fae")
    fae = as_date(row.get(fae_column)) if fae_column else None
    tokens = set(re.findall(r"[a-z0-9]+", program_normalized))
    if {"fae", "fas"} & tokens and entry and (today - entry).days > 120 and not fae:
        fragments.append(render(catalog, "CUMPLIMIENTO", "C08_FICHA_FAE", PNOMBRE=first)); ids.append("C-08")
    audience, audience_ids = _audience(record, columns, catalog, today); fragments += audience; ids += audience_ids
    if not principal:
        if fragments:
            fragments.insert(0, render(catalog, "CUMPLIMIENTO", "C09_BASE_BREVE")); ids.insert(0, "C-09")
        else:
            fragments.append(render(catalog, "CUMPLIMIENTO", "C09_SIN_OBSERVACIONES")); ids.append("C-09")
    return _compose(prefix(row, columns), fragments), tuple(ids), tuple(issues)
