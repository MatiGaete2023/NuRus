...
Versión **0.4.0.dev5**. Entrada: `Abrir_CSMP.bat`. Módulo: `nurus.personal.app`. El flujo visible tiene Trabajo, Correos, Resoluciones, Configuración y Enviados.
...
Resoluciones permite escoger `PC_IE`, `PC_INFO` o `NOMENCL`, agrupa por tribunal/RIT/tipo y genera un solo Word con página nueva por proyecto. Cuando una planilla revisada utiliza `RES`, la selección humana es autoritativa: solo los registros marcados afirmativamente se incorporan automáticamente y los descartados no reaparecen por una sugerencia previa del motor. Los casos adicionales se agregan mediante selección explícita en Trabajo. Varias personas del mismo grupo se integran en un solo proyecto y cada NNA queda inmediatamente asociado a su propia cédula de identidad. `FECHA` y `FECHA_RESOLUCION` se renderizan totalmente en palabras, incluidos día y año.
...
[G-ESTADO] Implementación personal consolidada en 0.4.0.dev5; no quedan validaciones de excepción de cruce visibles en el flujo personal.
