...
La release 0.4.0.dev5 conserva los contratos de sincronía entre `MATRIZ_REGLAS_PERSONAL.json` y `textos_base.json`, coherencia de versión/documentación y exclusión de `.venv-csmp`/artefactos de build. Agrega regresiones para fechas judiciales íntegramente en palabras, preparación integral de correos por modo + acciones, persistencia de `NURUS_REGLAS`, recuperación desde trazabilidad de exportaciones anteriores, prevalencia de la selección humana `RES` y correspondencia nombre/cédula para varios NNA de un mismo RIT. El run final debe consultarse sobre el HEAD de la rama.
